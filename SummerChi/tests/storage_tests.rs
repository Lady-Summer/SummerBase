...
mod core_tests {
    ...
    const MOCK_EDGES: Vec<&Edge<'a, f64>> = vec![
        Edge::<'a, f64>::new(&2, &2, &3, &4.0),
        Edge::<'a, f64>::new(&0, &0, &1, &2.0),
        Edge::<'a, f64>::new(&4, &5, &2, &8.0),
        Edge::<'a, f64>::new(&3, &2, &4, &1.0),
        Edge::<'a, f64>::new(&1, &1, &2, &3.0),
    ];
    ...
    impl <'a> EdgeDataShard<'a> {
        ...
        fn test_find_by_id(&mut self, edge_id: &u64) -> Option<&Edge<'a, f64>> {
    ...
    fn test_edge_shard_sort() {
        ...
        mock_edge_sorted.sort_by_key(|x| x.get_id());
    ...
    fn test_find_by_id() {
        ...
        mock_edge_shard.expect_test_find_by_id()
            .withf(|x: &u64| *x == 2)
            .returning(|x: Option<&Edge<'_, f64>>| x);
        let mut mock_edge_sorted = MOCK_EDGES.to_vec();
        mock_edge_sorted.sort_by_key(|x| x.get_id());
        assert_eq!(mock_edge_shard, EdgeDataShard(mock_edge_sorted))
    }
